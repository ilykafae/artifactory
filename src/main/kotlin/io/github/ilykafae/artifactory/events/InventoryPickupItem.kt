...
class InventoryPickupItem : Listener {
    ...
    fun onInventoryPickupItem(event: InventoryPickupItemEvent) {
        if (event.inventory.type == InventoryType.HOPPER && !Artifactory.serverConfig.get().allowArtifactInDispenser && event.item.itemStack.isArtifact() && !Artifactory.serverConfig.get().ignoredArtifacts.contains(event.item.itemStack.getArtifact()!!.id)) {
